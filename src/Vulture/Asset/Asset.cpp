...
namespace Vulture
{
	...
	Ref<Asset> AssetHandle::GetAsset() const
	{
	...
	Model* AssetHandle::GetModel() const
	{
		...
		return &reinterpret_cast<ModelAsset*>(m_Manager->GetAsset(*this).get())->Model;
	}
	...
	Image* AssetHandle::GetImage() const
	{
		...
		return &reinterpret_cast<TextureAsset*>(m_Manager->GetAsset(*this).get())->Image;
	}
